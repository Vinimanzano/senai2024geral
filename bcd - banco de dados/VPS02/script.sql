...
BEGIN
    UPDATE Aluguel a
    SET a.subtotal = (SELECT SUM(datediff(devolucao, retirada) * (SELECT diaria FROM Veiculo WHERE placa = a.placa)) AS subtotal FROM Aluguel WHERE id = a.id)
...
CREATE TRIGGER update_subtotal_after_update
AFTER UPDATE ON Aluguel
FOR EACH ROW
BEGIN
    IF NEW.devolucao IS NOT NULL THEN
        UPDATE Aluguel
        SET subtotal = (SELECT SUM(datediff(NEW.devolucao, NEW.retirada) * (SELECT diaria FROM Veiculo WHERE placa = NEW.placa)) AS subtotal)
        WHERE id = NEW.id;
    END IF;
END$$
